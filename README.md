...
無料の公開RSSからニュースを取得し、重要性・関連性・前回からの変化を整理したHTML/PDFを毎日作る学習プロジェクトです。**Version 1（Phase 0〜14）は完成済み**で、Version 2として公式記事本文に基づくAI日本語要約を追加しています。別途APIキーや有料ニュースAPIは使いません。GitHub Actionsの要約は、リポジトリ所有者のGitHub Copilot利用枠を使います。

## Phase 0: 環境確認

- 作業環境: macOS
- 確認したPython: 3.14.2
- 別途APIキー: 不要（自動日本語要約にはGitHub Copilotの利用権が必要）
- データ取得方法: 公開RSS / Atom
...
1. RSSのタイトルと概要を取得します。
2. リンク先がHTML、気象庁XML、日本銀行などのPDFの場合は、公開本文も要約材料として取得します。
3. GitHub Actions内でGitHub Copilot CLIを1回呼び出し、各記事を120〜220文字、2〜4文程度の日本語に要約します。
4. 要約が短すぎる、英語のまま、記事IDが欠けるなどの品質不良があれば、自動実行を失敗にして不完全なレポートの公開を防ぎます。

記事ページを取得できない場合はRSS本文を使います。要約は内容理解の補助であり、事実確認済みの原文ではありません。投資、法律、災害対応などの判断では必ずURLから原文を確認してください。
...
日本語要約はRSSと公式記事ページの本文を基にAIが作成します。AIによる事実確認、法律・投資判断は行いません。重要な判断では必ず原文と一次情報を確認してください。
...
- `日本語要約の生成に失敗`: GitHub Copilotの利用権・利用枠と、Actionsの`copilot-requests: write`権限を確認。
